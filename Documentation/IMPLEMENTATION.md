...
## Browser build and distribution

Run `bash Tools/build.sh web` with Unity 6000.6.0f1 Web Build Support installed. The helper selects the WebGL build target and IL2CPP, retains the runtime shader references, and writes `Builds/WebGL` plus [build evidence](../Artifacts/web-build-result.json). The custom template is in `Assets/WebGLTemplates/Seabright`. Gzip with Unity's decompression fallback works on GitHub Pages without custom response headers. `devicePixelRatio: 1` bounds Retina rendering cost, and a second click after loading activates browser audio. `autoSyncPersistentDataPath: true` persists ordinary save-file writes into browser storage. [Unity template configuration](https://docs.unity3d.com/ja/6000.0/Manual/web-templates-build-configuration.html).

The [browser check record](../Artifacts/web-browser-check.json) covers actual browser pointer/keyboard construction, visible first-home/shop/industry development, restored day/population/treasury and structures after reload, night/full-screen rendering and console checks. A local-only AnalyserNode wrapper observed a running Web Audio signal, zero output while muted and resumed output after unmuting; it is excluded from published files. This is a focused port check, not a repeat of the native 609-resident acceptance journey or a complete browser/device compatibility matrix. The Unity build report has zero errors/warnings; the raw compiler log also contains upstream Emscripten WebGPU/Firefox and deprecated-symbol notices.

The [v0.2.1 release](https://github.com/codersusu/game-city-skylines/releases/tag/v0.2.1) contains the already tested 9 September Apple Silicon Mac app and the 14 September web build. `python3 Tools/package-releases.py` checks the Mac signature/ZIP and web ZIP integrity, excludes the local audio QA page, and prepares both downloads with SHA-256 sums. [Release manifest](../Artifacts/release-manifest.json). Mac and browser saves use separate storage. The Mac app has an ad-hoc signature and is not notarized.

The `gh-pages` branch contains only the playable web output, README and bundled notices. GitHub Pages serves it at [the browser-play URL](https://codersusu.github.io/game-city-skylines/). For self-hosting, extract the WebGL ZIP and serve its root over HTTP(S). Browser saves are tied to the site's origin/path; moving the hosted game can create a separate save store. Clearing browser site data removes saves. Mobile/touch play is outside the verified scope.
